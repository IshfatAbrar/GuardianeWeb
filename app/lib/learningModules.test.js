...
describe("progressFor", () => {
  ...
  it("converts the child's 0-100 percent to a 0..1 fraction", () => {
    expect(
      progressFor(assignment(), progressMap({ c1_3: { progress: 50 } })),
    ).toBe(0.5);
    expect(
      progressFor(assignment(), progressMap({ c1_3: { progress: 33 } })),
    ).toBe(0.33);
    expect(
      progressFor(assignment(), progressMap({ c1_3: { progress: 100 } })),
    ).toBe(1);
  });
  ...
  it("rejects the NaN the child writes for a 0-lesson module", () => {
    ...
    const p = progressFor(
      assignment(),
      progressMap({
        c1_3: { progress: NaN, lessonsCompleted: 0, totalLessons: 0 },
      }),
    );
  ...
  it("joins across the number/string moduleId mismatch", () => {
    ...
    expect(
      progressFor(
        assignment({ moduleId: 3 }),
        progressMap({ c1_3: { progress: 100 } }),
      ),
    ).toBe(1);
  });
  ...
  it("falls back to lesson counts when progress is absent", () => {
    expect(
      progressFor(
        assignment(),
        progressMap({ c1_3: { lessonsCompleted: 2, totalLessons: 8 } }),
      ),
    ).toBe(0.25);
  });
  ...
  it("does not divide by zero when totalLessons is 0", () => {
    expect(
      progressFor(
        assignment(),
        progressMap({ c1_3: { lessonsCompleted: 0, totalLessons: 0 } }),
      ),
    ).toBe(0);
  });
  ...
  it("clamps out-of-range percents", () => {
    expect(
      progressFor(assignment(), progressMap({ c1_3: { progress: 500 } })),
    ).toBe(1);
    expect(
      progressFor(assignment(), progressMap({ c1_3: { progress: -10 } })),
    ).toBe(0);
  });
});
...
describe("isAssignmentCompleted", () => {
  it("trusts the child app's completed status", () => {
    expect(
      isAssignmentCompleted(
        assignment(),
        progressMap({ c1_3: { status: "completed" } }),
      ),
    ).toBe(true);
  });
...
describe("isAssignmentOverdue", () => {
  ...
  it("is overdue when past due and unfinished", () => {
    expect(
      isAssignmentOverdue(assignment({ dueDate: past }), progressMap({})),
    ).toBe(true);
  });
  ...
  it("is not overdue before the due date, or with no due date", () => {
    expect(
      isAssignmentOverdue(assignment({ dueDate: future }), progressMap({})),
    ).toBe(false);
...
describe("effectiveAssignmentStatus", () => {
  ...
  it("prefers completed over overdue", () => {
    ...
    expect(
      effectiveAssignmentStatus(a, progressMap({ c1_3: { progress: 100 } })),
    ).toBe(ASSIGNMENT_STATUS.COMPLETED);
  });
  ...
  it("reports overdue for unfinished past-due work", () => {
    const a = assignment({ dueDate: ts(Date.now() - 1000) });
    expect(
      effectiveAssignmentStatus(a, progressMap({ c1_3: { progress: 20 } })),
    ).toBe(ASSIGNMENT_STATUS.OVERDUE);
  });
});
