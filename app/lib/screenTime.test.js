...
describe("totalSeconds", () => {
  it("sums the per-row totals", () => {
    expect(
      totalSeconds([{ totalScreenTime: 100 }, { totalScreenTime: 50 }]),
    ).toBe(150);
  });
...
describe("aggregateApps", () => {
  it("sums the same app across days and sorts biggest first", () => {
    const rows = [
      {
        allApps: [
          { appName: "TikTok", packageName: "com.tiktok", timeSpent: 600 },
        ],
      },
      {
        allApps: [
  ...
  it("keys on packageName so a renamed display name still merges", () => {
    const rows = [
      {
        allApps: [
          { appName: "Instagram", packageName: "com.insta", timeSpent: 60 },
        ],
      },
      {
        allApps: [
  ...
  it("recomputes percentage across the range instead of reusing the daily one", () => {
    ...
    const rows = [
      {
        allApps: [
          { appName: "A", packageName: "a", timeSpent: 100, percentage: 100 },
        ],
      },
      {
        allApps: [
          { appName: "B", packageName: "b", timeSpent: 100, percentage: 100 },
        ],
      },
    ];
