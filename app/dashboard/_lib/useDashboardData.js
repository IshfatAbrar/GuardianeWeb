...
import {
  listenToChildrenForParent,
  listenToMoodHistoryForChild,
  listenToScreenTimeForChild,
} from "../../lib/database";
import {
  listenToAlerts,
...
export function useDashboardData() {
  ...
  useEffect(() => {
    ...
    const unsubScreenTime = listenToScreenTimeForChild(
      selectedChildId,
      (rows) => {
    ...
    const unsubInsights = listenToInsightsForChild(
      selectedChildId,
      (insight) => {
  ...
  const insights =
    insightState.childId === selectedChildId ? insightState.data : null;
  const insightsLoading =
    !!selectedChildId && insightState.childId !== selectedChildId;
  ...
  const assignmentCounts = useMemo(() => {
    const completed = assignments.filter((a) =>
      isAssignmentCompleted(a, visibleProgress),
    );
    const started = assignments.filter(
      (a) =>
        !isAssignmentCompleted(a, visibleProgress) &&
        progressFor(a, visibleProgress) > 0,
    );
